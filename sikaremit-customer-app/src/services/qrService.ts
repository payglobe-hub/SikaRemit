...
// Extend the shared QR service with customer-specific functionality
class CustomerQRService {
  ...
  async generateQRCode(data: QRGenerationData): Promise<QRCodeData> {
    try {
      const response = await api.post('/api/v1/payments/qr/generate/', data);
...
export const qrService = new CustomerQRService();

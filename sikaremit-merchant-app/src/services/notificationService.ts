...
const notificationService = {
  ...
  registerToken: async (token: string): Promise<boolean> => {
    try {
      await axios.post(`${API_BASE_URL}/api/v1/notifications/register-device/`, {
        token,
  ...
  unregisterToken: async (token: string): Promise<boolean> => {
    try {
      await axios.post(`${API_BASE_URL}/api/v1/notifications/unregister-device/`, { token }, {
        headers: await getAuthHeaders(),
      });
      return true;
    } catch (error) {
  ...
  markAsRead: async (notificationId: string): Promise<boolean> => {
    try {
      await axios.post(`${API_BASE_URL}/api/v1/notifications/${notificationId}/mark-read/`, {}, {
        headers: await getAuthHeaders(),
      });
      return true;
    } catch (error) {
  ...
  updatePreferences: async (preferences: Partial<NotificationPreferences>): Promise<boolean> => {
    try {
      await axios.patch(`${API_BASE_URL}/api/v1/notifications/preferences/`, preferences, {
        headers: await getAuthHeaders(),
      });
      return true;
    } catch (error) {

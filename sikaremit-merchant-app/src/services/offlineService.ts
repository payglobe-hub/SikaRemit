    ...
    try {
      ...
      switch (transaction.type) {
        case 'send_money': {
          ...
          await paymentService.sendMoney(transaction.data as any);
          break;
        }
        case 'airtime': {
          const mobileMoneyService = (await import('./mobileMoneyService')).default;
          await mobileMoneyService.buyAirtime(transaction.data as any);
          break;
        }
        case 'data_bundle': {
          const mobileMoneyService = (await import('./mobileMoneyService')).default;
          await mobileMoneyService.buyDataBundle(transaction.data as any);
          break;
        }
        case 'bill_payment': {
          const billPaymentService = (await import('./billPaymentService')).default;
          await billPaymentService.payBill(transaction.data as any);
          break;
        }
        case 'remittance': {
          const { paymentService } = await import('./paymentService');
          await paymentService.sendRemittance(transaction.data as any);
          break;
        }
        default:

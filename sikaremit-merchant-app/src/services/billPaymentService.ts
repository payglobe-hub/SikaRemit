...
const billPaymentService = {
  ...
  validateAccount: async (
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/api/v1/payments/bills/validate/`, {
      ...
      return {
        valid: false,
        message: apiError.response?.data?.message || 'Account validation failed',
      };
    }
  },
  ...
  getAccountDetails: async (
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/api/v1/payments/bills/account/`, {
        params: {
          provider_id: providerId,
          account_number: accountNumber,
        },
      ...
      const apiError = error as ApiError;
      console.warn('Failed to get account details:', apiError);
      return null;
    }
  },
  ...
  payBill: async (request: BillPaymentRequest): Promise<BillPaymentResponse> => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/api/v1/payments/bills/pay/${request.providerId}`, {
      ...
      return {
        ...
        reference,
        status: 'failed',
        message: apiError.response?.data?.message || 'Bill payment failed. Please try again.',
      };
    }
  },
  ...
  getPaymentHistory: async (providerId?: string): Promise<BillPaymentHistoryItem[]> => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/v1/payments/bills/`, {
        params: {
          provider_id: providerId,
          limit: 20,
        },
  ...
  getSavedAccounts: async (): Promise<BillAccount[]> => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/v1/payments/bills/saved-accounts/`, {
        headers: await getAuthHeaders()
      });
      return response.data.data || [];
    } catch (error: unknown) {
  ...
  saveAccount: async (
    ...
    try {
      await axios.post(`${API_BASE_URL}/api/v1/payments/bills/saved-accounts/`, {
        provider_id: providerId,
        account_number: accountNumber,
        nickname,
      }, {
  ...
  deleteSavedAccount: async (accountId: string): Promise<boolean> => {
    try {
      await axios.delete(`${API_BASE_URL}/api/v1/payments/bills/saved-accounts/${accountId}/`, {
        headers: await getAuthHeaders()
      });
      return true;
    } catch (error: unknown) {
  ...
  checkTransactionStatus: async (reference: string): Promise<BillPaymentResponse> => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/v1/payments/bills/${reference}/status/`, {
        headers: await getAuthHeaders(),
      });

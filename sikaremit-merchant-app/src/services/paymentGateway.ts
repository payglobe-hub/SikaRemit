...
import api from '@sikaremit/mobile-shared';
...
export type MobileMoneyProvider = 'mtn' | 'g_money' | 'airteltigo' | 'telecel';
...
const paymentGateway = {
  ...
  initializeCardPayment: async (
    ...
    try {
      const response = await api.post('/api/v1/payments/wallet/deposit/card/', {
        email,
        amount,
        reference,
        metadata,
